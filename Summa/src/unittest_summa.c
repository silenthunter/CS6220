...
void
random_matrix_test (int m, int n, int k, int px, int py, int panel_size)
{
  int num_procs = px * py;
  int rank = 0;
  ...
  if (rank == 0)
    {
      printf ("random_matrix_test m=%d n=%d k=%d px=%d py=%d............", m,
	      n, k, px, py);
...
main (int argc, char *argv[])
{
  ...
  random_matrix_test (8, 8, 8, 4, 2, 1);
  random_matrix_test (16, 16, 16, 8, 2, 1);
