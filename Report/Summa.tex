...
\title{Comparison of MPI, OpenMP and CUDA on the SUMMA algorithm}
...
\begin{figure}[tbh]
\includegraphics[scale=.45]{Images/Comms.png}
...
\subsection{CUDA}
\subsubsection{Host}
The CUDA implementation expands upon the SUMMA algorithm by replacing the normal local matrix multiply with one that computes the results on the GPU, rather than the CPU.
...
The shared memory is first divided into two arrays. One for $A$ and the other for $B$. Each thread then copies the matrix element it is responsible for from  global memory to the shared arrays for $A$ and $B$. These reads are made in a way that allow for coalesced memory access as per \cite{CUDA_PGUIDE}, resulting in a minimal number of memory transactions. There is a $\_\_syncthread()$ call after this procedure to ensure the shared memory is in place before proceeding.
...
\section{Evaluation}
The quality of the implementations is judged primarily on GFLOPS/s and future scalability. GFLOPS/s is acquired by comparing each implementations runtime against the GFLOPS of computing a matrix (Eqn \ref{eqn:GFLOPS}). This number is also compared against the theoretical maximum GFLOPS of the given device.
...
\begin{figure}[tbh]
\includegraphics[scale=.25]{Images/SummaPerf.png}
...
This implementation performed worse than I was expecting. It only reaches about 50\% of the SUMMA implementation. I would speculate this is due to cache misses. When MPI enters this method, it proceeds sequentially. The column that was just used will stay in cache as the inner loop iterates through the rows. OpenMP threads will not necessarily be assigned in order. This will lead to more cache conflicts, and the cache-miss penalty may be high.

The SUMMA implementation had a MPI process per core totalling 16 process on Jinx fourcore, while the OpenMP solution has one MPI process per Jinx node, and one OpenMP thread per core. Each implementation has 16 threads total, and have no real benefit over each other in terms of processing power.

\subsection{CUDA}
The performance of the CUDA+SUMMA implementation was signficant after seeing the results of the previous two implementations. While very little benefit is gained from low panel block sizes (Tbl \ref{tbl:CUDA_PERF}), the more data that can be provided to the GPU, the better the performance was. It not only continued to scale as the panel block size increased, but the GFLOPS increased at a rate greater than any other implementation.

\begin{figure}[htb]
\includegraphics[scale=.25]{Images/CUDAPerf.png}
...
\begin{figure}[htb]
\includegraphics[scale=.25]{Images/CUDASpeedup.png}
